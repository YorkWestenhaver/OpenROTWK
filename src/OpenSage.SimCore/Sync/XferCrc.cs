...
//
// Clean-room: this is the behavioral spec of crc-byteorder.md §4, recovered at the instruction
// level from the shipped game.dat (SHA-256 d98777e1...) - no engine code was copied. The
// load-bearing facts, each pinned by a test in XferCrcTests:
//
//   * init value 0; per step: hibit = crc >> 31 (logical, 0/1); crc = crc*2 + hibit + value,
//     unsigned 32-bit wrap - i.e. rotate-left-1 then add (word-loop op at 0xa21214-0xa2121a).
//   * words are read from the buffer as NATIVE LITTLE-ENDIAN dwords; there is no htonl, no
//     byteswap (the decisive `03 01 ADD EAX,[ECX]` at 0xa2121a). Byte composition below is
//     explicit, so the fold is byte-identical on strict-alignment targets too.
...
//   * null buffer + non-zero length folds nothing (0xa211e2/0xa211f1). Spans cannot express
//     that state; callers with raw pointers must apply the guard before constructing the span.
...
namespace OpenSage.SimCore.Sync
{
    ...
    public struct XferCrc
    {
        ...
        /// the original's seed-fold helper (crc-byteorder §2.1: FUN_007ed74f is byte-wise by

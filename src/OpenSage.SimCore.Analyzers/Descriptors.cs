...
internal static class Descriptors
{
    ...
    // Info + enabled-by-default: it never fails a build (Info is immune to TreatWarningsAsErrors),
    // so it can ride along on OpenSage.Game today while the R13 fixer wave raises it to warning/
    // error deliberately, per-directory, via .editorconfig as modules are cleaned up.
    private static DiagnosticDescriptor Info(string id, string category, string title, string format) =>
        new DiagnosticDescriptor(id, title, format, category, DiagnosticSeverity.Info, true, null, HelpUri);
    ...
    /// <summary>
    /// Xfer completeness (api-freeze-v1 S4 per-object Xfer walk): a module field that is mutated
    /// as sim state but never touched by the serialization walk (Load/Save/Persist/Xfer) makes
    /// save/load and cross-machine lockstep diverge. The R12 review swarm found this repeatedly -
    /// the canonical case is TurretAIUpdate never persisting its turret state machine. Info +
    /// opt-in so the existing backlog does not fail the build before the fixer wave lands.
    /// </summary>
    public static readonly DiagnosticDescriptor UnpersistedMutableField = Info(
        "SIMCORE011", Determinism,
        "Mutable module state is not persisted in the Xfer/Load/Save walk",
        "field '{0}' is mutated but never persisted in {1}; save-load and lockstep will diverge - persist it in the serialization walk, or mark it [NotXfered] if it is a rebuildable cache");
}

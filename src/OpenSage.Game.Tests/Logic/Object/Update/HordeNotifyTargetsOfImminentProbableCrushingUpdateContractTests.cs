...
// plus the shadow-copy base test and a mid-state save/load round-trip continuation. Object
// definitions are parsed from INI text through the real parser. Mirrors the landed base variant
// (NotifyTargetsOfImminentProbableCrushingUpdateContractTests) with an authorable ScanWidth=60
// substituted for the base's DefaultScanWidth=40.
...
public class HordeNotifyTargetsOfImminentProbableCrushingUpdateContractTests
{
    // Cavalry horde: CrusherLevel 2 (can crush CrushableLevel < 2). ScanWidth = 60.
...
";
    ...
    private static HordeNotifyTargetsOfImminentProbableCrushingUpdate ModuleOf(GameObject obj) =>
        obj.BehaviorModules.OfType<HordeNotifyTargetsOfImminentProbableCrushingUpdate>().Single();
    ...
    [Fact]
    public void ScanWidthField_ParsesFromIni()
    {
        var game = NewGame();
        var cavalry = game.SpawnObject("CavalryHorde", game.CivilianPlayer, new Vector3(0, 0, 0));
        var data = Assert.IsType<HordeNotifyTargetsOfImminentProbableCrushingUpdateModuleData>(
            cavalry.Definition.Behaviors["ModuleTag_NotifyCrushScan"].Data);

        Assert.Equal(new OpenSage.SimCore.Numerics.Fix64(60), data.ScanWidth);
    }
    ...
    public void CrusherInScanWidth_WarnsInfantryTarget()
    {
        var game = NewGame();
        var cavalry = game.SpawnObject("CavalryHorde", game.CivilianPlayer, new Vector3(0, 0, 0));
        var infantry = game.SpawnObject("Infantry", game.PlayerManager.NeutralPlayer, new Vector3(50, 0, 0));
    ...
    [Fact]
    public void TargetJustInsideScanWidthBoundary_IsWarned_ExactlyAtOrBeyond_IsNot()
    {
        // The partition query is a strict-< range test (WithinStrict, GPL
        // iterateObjectsInRange FROM_CENTER_2D semantics - see SimPartitionGrid.QueryCore):
        // a target exactly at ScanWidth distance is already outside the open interval, so the
        // deterministic boundary is "just inside" vs "at-or-beyond", not an inclusive edge.
        var game = NewGame();
        var cavalry = game.SpawnObject("CavalryHorde", game.CivilianPlayer, new Vector3(0, 0, 0));
        var justInside = game.SpawnObject("Infantry", game.PlayerManager.NeutralPlayer, new Vector3(59, 0, 0));
        var atBoundary = game.SpawnObject("Infantry", game.PlayerManager.NeutralPlayer, new Vector3(60, 0, 0));
        MakeEnemies(game.CivilianPlayer, game.PlayerManager.NeutralPlayer);

        StepFrames(game, 2);

        Assert.True(Braced(justInside));
        Assert.False(Braced(atBoundary));
    }
    ...
    public void MultipleTargetsInScanWidth_AllWarnedSimultaneously()
    {
        var game = NewGame();
        var cavalry = game.SpawnObject("CavalryHorde", game.CivilianPlayer, new Vector3(0, 0, 0));
        var infantryA = game.SpawnObject("Infantry", game.PlayerManager.NeutralPlayer, new Vector3(20, 0, 0));
        var infantryB = game.SpawnObject("Infantry", game.PlayerManager.NeutralPlayer, new Vector3(-30, 10, 0));
        MakeEnemies(game.CivilianPlayer, game.PlayerManager.NeutralPlayer);
    ...
    {
        var game = NewGame();
        game.SpawnObject("CavalryHorde", game.CivilianPlayer, new Vector3(0, 0, 0));
        var farInfantry = game.SpawnObject("Infantry", game.PlayerManager.NeutralPlayer, new Vector3(500, 0, 0)); // > ScanWidth (60)
        MakeEnemies(game.CivilianPlayer, game.PlayerManager.NeutralPlayer);
    ...
    {
        var game = NewGame();
        game.SpawnObject("CavalryHordeNoModule", game.CivilianPlayer, new Vector3(0, 0, 0));
        var infantry = game.SpawnObject("Infantry", game.PlayerManager.NeutralPlayer, new Vector3(10, 0, 0));
        MakeEnemies(game.CivilianPlayer, game.PlayerManager.NeutralPlayer);
    ...
    {
        var game = NewGame();
        var liveHost = game.SpawnObject("CavalryHorde", game.CivilianPlayer, new Vector3(0, 0, 0));
        ...
        var shadowHost = game.SpawnObject("CavalryHorde", game.CivilianPlayer, new Vector3(400, 400, 0));
